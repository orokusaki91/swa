...
@section('content')
	<form action="{{ url('admin/pages/update') }}" method="post" enctype="multipart/form-data" >
        ...
        <h3 class="heading">Hintergrundbild Home</h3>
        @if($images->count() > 0)
            @foreach($images as $image)
                <img src="{{ asset('storage/uploads/home/' . $image->path) }}" class="ap_pi large-img" />
            @endforeach
        @endif
        ...
        <label class="d-block">Text Hintergrundbild</label>
        <textarea name="header_text_1">{{ $pageContent['header_text'] }}</textarea>

        <label class="d-block">Mehr erfahren Text</label>
