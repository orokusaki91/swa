...
<form action="{{ url('admin/pages/' . $page->slug . '/' . $partner->id . '/update') }}" method="post" enctype="multipart/form-data">
    ...
    <label class="mr-1">Bild</label>
    @if($partner->images->count() > 0)
        <img src="{{ asset('storage/uploads/' . $page->slug . '/' . $partner->images->first()->path) }}" class="ap_pi large-img">
    @endif
    ...
    <label class="mr-1">Überschrift</label>
    <input type="text" name="title_1" value="{{ $partner->title }}">
    <label class="mr-1">Webseite</label>
